...
RUN echo "(ql:quickload :hdf5-cffi)" >> /root/.sbclrc
RUN echo "(ql:quickload :alcove)" >> /root/.sbclrc
RUN echo "(use-package '(:cffi :alcove) :cl-jupyter-user)" >> /root/.sbclrc
RUN echo "(shadow '(off-t size-t))" >> /root/.sbclrc
RUN echo "(use-package :hdf5 :cl-jupyter-user)" >> /root/.sbclrc
